...
class PlayerService : MediaBrowserServiceCompat() {
     ...
     */
    private fun updateMediaMetadata(track: app.async.core.model.SearchResult) {
        ...
        override fun onSetRepeatMode(repeatMode: Int) {
            ...
            when (repeatMode) {
                ...
                PlaybackStateCompat.REPEAT_MODE_GROUP -> {
                    // Group repeat mode - treat as ALL mode
                    playbackManager.toggleRepeat()
                }
                PlaybackStateCompat.REPEAT_MODE_INVALID -> {
                    // Invalid mode - do nothing or set to default
                    logcat { "Invalid repeat mode received: $repeatMode" }
                }
                else -> {
                    // Handle any other unknown modes
                    logcat { "Unknown repeat mode: $repeatMode" }
                }
            }
        }
